...
public:
  ...
  void initialize(const vk::PhysicalDeviceMemoryProperties& mem_props) {
    ...
    buf_is_changed = gpu::create_exclusive_buffer<uint32_t>(device, num_blocks, vk::BufferUsageFlagBits::eStorageBuffer);
    buf_was_changed = gpu::create_exclusive_buffer<uint32_t>(device, num_blocks, vk::BufferUsageFlagBits::eStorageBuffer);

    mem_dist = gpu::alloc_and_bind(device, mem_props, buf_dist, vk::MemoryPropertyFlagBits::eHostVisible);
    mem_is_changed = gpu::alloc_and_bind(device, mem_props, buf_is_changed, vk::MemoryPropertyFlagBits::eHostVisible);
    mem_was_changed = gpu::alloc_and_bind(device, mem_props, buf_was_changed, vk::MemoryPropertyFlagBits::eHostVisible);

    gpu_dist = (uint32_t *)device.mapMemory(mem_dist, 0, num_nodes * sizeof(uint32_t));
    gpu_changed = (uint32_t *)device.mapMemory(mem_is_changed, 0, sizeof(uint32_t));
  }
