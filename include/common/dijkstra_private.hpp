#ifndef GPUSSSP_COMMON_DIJKSTRA_PRIVATE_HPP
#define GPUSSSP_COMMON_DIJKSTRA_PRIVATE_HPP
...
template <typename T>
struct has_node_weights<T, decltype((void)T::node_weight_penalty, 0)> : std::true_type
{
};

template <typename PolicyT, typename NodePotentialsT>
void insert_label(typename PolicyT::queue_t &queue,
                  NodeLabels<PolicyT> &labels,
                  const PolicyT &policy,
                  const NodePotentialsT &potentials,
                  const typename PolicyT::node_id_t target,
                  typename PolicyT::label_t label)
{
